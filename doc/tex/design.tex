...
% @todo: добавить на основе чего и почему свой алгоритм
...
\item $E = \{ e = (u, v, color) :  u, v \in V \land c \in C \}$ --- множество дуг.
\end{itemize}
...
%
$$\textrm{NEIGHBORS}(v_G) = \{ u : (u \ne v_G) \land ( ( \exists (u, v_G, color^i_G) \in E_G ) | ( \exists (v_G, v, color^j_G) \in E_G ) ) \}$$
...
$\textrm{COMPONENTS}(G) = \{ (V, Color, E) : V \subseteq V_G \land Color \subseteq Color_G \land E \subseteq E_G \land (\forall u, v \in V)(\exists \{ w_1 = u, \dots, w_n, \dots, w_N = v : (\forall w_n, n=\overline{2,N})(w_{n-1} \in \textrm{NEIGHBORS}(w_n)) \})$
...
\item $E = \{ (u, v, c) : u, v \in V \land c \in Color \}$ --- множество дуг
\end{itemize}
...
$\textrm{DIRECT}(op, classifiers) = \{ x : x = op \land (\exists g \in classifiers)(x \in g.operations) \}$
