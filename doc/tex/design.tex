...
\textbf{Определение}. ADD --- процедура, добавляющаяя конфигурацию в
коллекцию конфигураций. Реализация зависит от типа коллекции.

\textbf{Определение}. $\textrm{TAKE} : 2^C \to C$ --- извлекает из коллекции конфигураций
...
\Function{initial\_configurations}{$V_t$, $V_p$}
    ...
    \State $E \gets \{ ( v^n_t, v^n_p ) : ( v^n_t \cong v^n_p ) \And ( \forall i, j) ( v^i_p \ne v^j_p ) \And ( v^n_t \in V_t ) \And ( v^n_p \in V_p ) \}$
    \ForAll{$( v_t, v_p ) \in E$}
        \State $neighbors_t \gets$ \Call{neighbors}{$v_t$}
        \State $neighbors_p \gets$ \Call{neighbors}{$v_p$}
        \ForAll{$chain \in$ \Call{chains}{$neighbors_t$, $neighbors_p$} }
...
\Function{advance}{$C$}
    ...
    \While{$true$}
        ...
        \State $C_{new} \gets (selected, current, checked)$
        \If{$current > |selected|$}
            \State \Return $C_{new}$
        \EndIf
        \State $(target, pattern) \gets selected^{current}$
        \If{$target \not\in checked\_t \And pattern \not\in checked\_p \And$ \Call{is\_valid}{$C_{new}$}}
            \State \Return $C_{new}$
        \EndIf
    \EndWhile
\EndFunction
...
\Function{match\_one}{$G_{target}$, $G_{pattern}$}
    ...
    \While{$Configurations \ne \varnothing$}
        \State $C \gets$ \Call{take}{$Configurations$}
        ...
        \If{$C$ --- конечная}
            ...
            \EndIf
        \Else
            \ForAll{$C_{new} \in$ \Call{derived\_configurations}{$C$}}
                \State \Call{add}{$Configurations$, $C_{new}$}
            \EndFor
        \EndIf
    \EndWhile
    \State \Return $result$
